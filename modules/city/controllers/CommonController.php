...
//        exit;
    ...
    public function out_stream($title,$data,$filename,$w=array(),$type = 0){
        ...
        $name = iconv("utf-8", "utf-8", './Data/excel/'.$filename.'.xlsx');
        $objExcel->getActiveSheet()->getHeaderFooter()->setOddHeader('&L&BPersonal cash register&RPrinted on &D');
        $objExcel->getActiveSheet()->getHeaderFooter()->setOddFooter('&L&B' . $objExcel->getProperties()->getTitle() . '&RPage &P of &N');
        $objWriter = \PHPExcel_IOFactory::createWriter($objExcel, 'Excel5');
        if ($type == 0) {
...
//        elseif($dayNum==$curDayNum) $timeFlag="";
     ...
     * 计算公里数
     * */
    function mileage_interval($type,$km,$scale){
        // 查询里程系数标准
        $result = $scale;
        // 获取0-100里程系数
    ...
    /*
        *合并订单统计重量，体积，数量
        * */
    public function get_count($id,$name){
        $arr = AppCity::find()->where(['in','id',$id])->sum($name);
        return $arr;
    }
     ...
     * 验证司机是否有操作权限
     * */
    public function get_auth($phone,$id){
        $driver = AppOrder::findOne($id);
        if ($phone != $driver->driver_phone){
            $data = $this->encrypt(['code'=>401,'msg'=>'无权限操作']);
            echo json_encode($data);
            exit;
        }
