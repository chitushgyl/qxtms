...
class CommonController extends Controller
{
    ...
    public function init(){
        if(in_array(Yii::$app->requestedRoute,[
            'user/register_company','user/register','user/login'
        ])){
            self::controllerLimit([
                ['funciton' => 'user/register_company',
                    'time_limit' => 120,
                    'try_times' => 1,
                ],[
                    'funciton' => 'user/register',
                    'time_limit' => 120,
                    'try_times' => 1,
                ],[
                    'funciton' => 'user/login',
    ...
    // 左边菜单栏
    public function left_auth_two($user){
        $tree = $auth = [];
        // 公司账户 只有主账号才能看到
        if ($user->admin_id == 1 && $user->com_type == 1) {
            $parent_group_id = $user->parent_group_id;
            // $group = AppGroup::findOne($parent_group_id);
            $level = AppLevel::findOne(2);
            $auth = $level->auth;
            $role_ids = explode(',',$auth);
            $auth = AppAuthLeft::find()
                ->select(['id','display_name','route','parent_id','icon','num'])
...
//            $body = createContent('woyebuzhidao',$data['content']);
    ...
    * */
    public function detailadd($id){
        if ($id){
            $result1 =  District::find()->where(array('id'=>$id))->one();
            return $result1->name;
        }else{
            return '';
        }
    ...
    public function Upload($models,$file)
     {
         if(isset($file)) {
             ...
             $data = $this->encrypt(['code'=>401,'msg'=>'上传文件不能为空']);
             echo $this->resultInfo($data);
             exit;
         }
     }    
     /*
...
//        elseif($dayNum==$curDayNum) $timeFlag="";
     ...
     *
     * */
    public function count_kilo($startcity,$endcity){
        if (count($startcity) == 1 && count($endcity) == 1){
            // 起点城市经纬度
            $start_action = bd_local($type='2',$startcity[0]['city'],$startcity[0]['area'].$startcity[0]['info']);//经纬度
            // 终点城市经纬度
            $end_action = bd_local($type='2',$endcity[0]['city'],$endcity[0]['area'].$endcity[0]['info']);//经纬度
            ...
            for ($i=1;$i<count($startcity);$i++){
                $start_action = bd_local($type='2',$startcity[$i-1]['city'],$startcity[$i-1]['area'].$startcity[$i-1]['info']);//经纬度
                $start_action1= bd_local($type='2',$startcity[$i]['city'],$startcity[$i]['area'].$startcity[$i]['info']);
                // 获取百度返回的结果
            ...
            $start_action2 = bd_local($type = '2',end($startcity)['city'],end($startcity)['area'].end($startcity)['info']);
            $end_action = bd_local($type = '2',$endcity[0]['city'],$endcity[0]['area'].$endcity[0]['info']);//经纬度
            ...
            for ($i=1;$i<count($endcity);$i++){
                $end_action = bd_local($type='2',$endcity[$i-1]['city'],$endcity[$i-1]['area'].$endcity[$i-1]['info']);
                $end_action1 = bd_local($type='2',$endcity[$i]['city'],$endcity[$i]['area'].$endcity[$i]['info']);
                $list = direction($end_action['lat'], $end_action['lng'], $end_action1['lat'], $end_action1['lng']);
                $finally = $list['distance']/1000;
                $km1 = $this->mileage_interval(2,(int)$finally);
                $km += $km1;
            }
            $end_action2 = bd_local($type='2',$endcity[0]['city'],$endcity[0]['area'].$endcity[0]['info']);
            $start_action = bd_local($type='2',$startcity[0]['city'],$startcity[0]['area'].$startcity[0]['info']);//经纬度
            ...
            for ($j=1;$j<count($endcity);$j++){
                $end_action = bd_local($type='2',$endcity[$j-1]['city'],$endcity[$j-1]['area'].$endcity[$j-1]['info']);
                $end_action1 = bd_local($type='2',$endcity[$j]['city'],$endcity[$j]['area'].$endcity[$j]['info']);
            ...
            $end_action3 = bd_local($type='2',$endcity[0]['city'],$endcity[0]['area'].$endcity[0]['info']);
            $start_action3 = bd_local($type='2',end($startcity)['city'],end($startcity)['area'].end($startcity)['info']);//经纬度
