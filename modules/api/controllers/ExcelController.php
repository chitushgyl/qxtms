...
//                $receive['create_user_name'] = $user->name;
            ...
            try{
                $res = Yii::$app->db->createCommand()->batchInsert(AppOrder::tableName(), ['ordernumber','takenumber','group_id','startcity','endcity','time_start','time_end','cartype','company_id','paytype', 'name','temperture','number','weight','volume','startstr','endstr','picktype','pickprice','sendtype','sendprice','price','more_price','otherprice','remark','order_type','create_time','update_time','total_price','money_state'], $info)->execute();
...
//                    exit();
                ...
                $start_arr = ['pro'=>$start_pro,'city'=>$start_city,'area'=>$start_area,'info'=>$start_info,'contant'=>$value['M'],'tel'=>$value['N']];
                $save_start_arr[] = $start_arr;
                $arr['startstr'] = json_encode([$start_arr],JSON_UNESCAPED_UNICODE);
...
//                $arr['more_price'] = (float)$value['AA'];
                ...
                $arr['create_time'] = $arr['update_time'] = date('Y-m-d H:i:s',time());
                $arr['total_price'] = (float)$arr['pickprice'] + (float)$arr['sendprice'] + $arr['price'] + $arr['otherprice'];
                $arr['money_state'] = 'N';
                $info[] = $arr;
...
//                $receive['create_user_name'] = $user->name;
            ...
            $transaction= AppOrder::getDb()->beginTransaction();
            try{
                $res = Yii::$app->db->createCommand()->batchInsert(AppOrder::tableName(), ['ordernumber','takenumber','group_id','startcity','endcity','time_start','company_id','paytype', 'name','temperture','number','weight','volume','startstr','endstr','picktype','pickprice','sendtype','sendprice','price','otherprice','remark','order_type','create_time','update_time','total_price','money_state'], $info)->execute();
                $res_r = Yii::$app->db->createCommand()->batchInsert(AppReceive::tableName(), ['compay_id','receivprice','trueprice','receive_info','group_id','paytype','ordernumber','type','create_time','update_time'], $receive_info)->execute();
                $arr = $this->insert_id($receive_info);
                if ($res && $arr && $res_r){
                    $transaction->commit();
