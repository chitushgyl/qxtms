...
class LineLogController extends CommonController
{
    ...
    public function actionIndex()
    {
        ...
        if ($city) {
            $list->orWhere(['like','a.startcity',$city])
                ->orWhere(['like','a.endcity',$city])
                ->orWhere(['like','a.centercity',$city]);
        }

        $list->andWhere(['a.group_id'=>$group_id,'a.delete_flag'=>'Y']);
        $count = $list->count();
...
//            }
    ...
    public function actionCopy_line(){
        ...
        $this->check_group_auth($model['group_id'],$user);
        $model['copy_id'] = $model['id'];
        unset($model['id']);
        $model['carriage_id'] = '';
        $line = new AppLineLog();
        $line->attributes = $model;
        $line->create_user_id = $user->id;
        $line->expire_time = $model['expire_time'];
