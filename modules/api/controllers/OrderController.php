...
//            }
    ...
    public function actionView(){
        ...
        if ($id) {
            $model = AppOrder::find()
                ->alias('a')
                ->select('a.*,b.name username,b.tel')
                ->leftJoin('app_admin b','a.deal_user = b.id')
                ->where(['a.id'=>$id])
                ->asArray()
                ->one();
...
//            生成运单，应收赤途，order表保存车辆信息更改订单状态
    ...
    public function actionPlatform_take(){
        ...
        if($order->money_state == 'N'){
            ...
            $payment->pay_type = 5;
            $res_p = $payment->save();
        }else if($order->money_state == 'Y' && $order->pay_status == 2){
            $receive->company_type = 2;
    ...
    public function actionSystem_take(){
        ...
        if ($order->money_state == 'N' || !$order->money_state){
            ...
            $payment->pay_type = 5;
            $res_p = $payment->save();
        }elseif($order->money_state == 'Y' && $order->pay_status == 2){
            $receive->company_type = 2;
            $receive->compay_id = 25;
            $receive->trueprice = 0;
        }
...
//        $this->check_group_auth($order->deal_company,$user);
    ...
    public function actionCopy_order(){
        ...
        if(in_array($order['order_status'],[3,4,5,6])){
            $data = $this->encrypt(['code'=>400,'msg'=>'订单承运中']);
            return $this->resultInfo($data);
        }
...
//          }
          ...
          if(empty($take_order->driverinfo)){
              $data = $this->encrypt(['code'=>400,'msg'=>'请确认车辆信息']);
              return $this->resultInfo($data);
          }
    ...
    public function actionCancel_order()
    {
        ...
        if (empty($token) || !$id){
            ...
            $this->check_group_auth($order->group_id,$user);
            if($order->line_status == 2){
                $data = $this->encrypt(['code'=>400,'msg'=>'订单已上线不可以取消']);
                return $this->resultInfo($data);
            }
            if ($order->order_status == 8){
                $data = $this->encrypt(['code'=>400,'msg'=>'订单已取消']);
            ...
            if ($order->copy != 1){
                $data = $this->encrypt(['code'=>400,'msg'=>'订单已开始调度,不可以取消']);
                return $this->resultInfo($data);
            }
    ...
    * */
    ...
    public function actionSelect_line(){
        ...
        $order = AppOrder::findOne($id);
        $line = AppLine::find()
            ->where(['group_id'=>$user->group_id,'startcity'=>$order->startcity,'endcity'=>$order->endcity,'delete_flag'=>'Y','state'=>1])
            ->andWhere(['>=','start_time',date('Y-m-d H:i:s',strtotime($order->time_start)+3600)])
            ->asArray()
            ->all();
        if(empty($line)){
            $data = $this->encrypt(['code'=>200,'msg'=>'没有符合要求的线路,建议上线处理','data'=>2]);
            return $this->resultInfo($data);
        }else{
            $data = $this->encrypt(['code'=>200,'msg'=>'继续操作','data'=>1]);
            return $this->resultInfo($data);
        }
    }
    ...
    public function actionOnline_order(){
        $input = Yii::$app->request->post();
        $id = $input['id'];
        $token = $input['token'];
        $line_price = $input['line_price'];
...
//        $temperture = array_unique($temperture,SORT_REGULAR);
        ...
        try {
                 ...
                 if ($list->main_order == 1){
                     if ($list->line_id){
                         if ($list->order_type != 7 && $list->order_type != 10){
                             ...
                             $list_main->save();
                         }else if($list->order_type == 7 || $list->order_type == 10){
            ...
            if ($arr && $lists && $res && $carriage && $res_pick){
                if($order_type == 2){
                    $bulk = new AppBulk();
                    $bulk->ordernumber = $pick_order->ordernumber;
                    $bulk->number = $pick_order->number;
                    $bulk->number1 = $pick_order->number1;
                    $bulk->weight = $pick_order->weight;
                    $bulk->volume = $pick_order->volume;
                    $bulk->shiftid = $shiftid;
                    $bulk->begincity = $pick_order->startcity;
                    $bulk->endcity = $pick_order->endcity;
                    $bulk->lineprice = $pick_order->price;
                    $bulk->picktype = $pick_order->picktype;
                    $bulk->sendtype = $pick_order->sendtype;
                    $bulk->begin_info = $pick_order->startstr;
                    $bulk->end_info = $pick_order->endstr;
                    $bulk->group_id = $pick_order->group_id;
                    $bulk->total_price = $pick_order->price;
                    $bulk->orderstate = 3;
                    $bulk->line_type = 1;
                    $bulk->save();
                }
    ...
    public function actionSelect_line_log(){
        $input = Yii::$app->request->post();
        $token = $input['token'];
        $ids = $input['id'];
        ...
        foreach ($ids as $key =>$value){
            ...
            if($line_type == 1){
                ...
                $line = AppLine::find()
                    ...
                    ->andWhere(['>=','a.start_time',date('Y-m-d H:i:s',strtotime($order->time_start)+3600)])
    ...
    public function actionCount_price(){
        ...
        $picktype = $input['picktype'];
        $sendtype = $input['sendtype'];
        $shift = AppLineLog::findOne($shiftid);
        $line = AppLineLog::findOne($shift->copy_id);
        ...
        if($picktype == 1){
            if ($weight >= $line->freepick){
                $lineprice = $lineprice;
            }else{
                $lineprice = $lineprice + $line->pickprice;
            }
        }
        if ($sendtype == 1){
            $lineprice = $lineprice + $line->sendprice;
        }
...
//        $user= $check_result['user'];
    ...
    public function actionCancel_dispatch(){
        ...
        $bulk = AppBulk::find()->where(['ordernumber'=>$order->ordernumber])->one();
    ...
    public function actionCarriage_list(){
       ...
       $check_result = $this->check_token_list($token,$chitu);//验证令牌
        $list = AppMegerOrder::find()
            ...
            ->leftJoin('app_line c','v.shiftid = c.id')
...
//            return $this->resultInfo($data);
    ...
    public function actionBulk_order(){
        ...
        $bulk = new AppBulk();
        $line = AppLine::findOne($shiftid);
        if($line->group_id == $user->group_id){
            $data = $this->encrypt(['code'=>'400','msg'=>'不可以在自己公司线路上下单']);
            return $this->resultInfo($data);
        }
        $transaction= AppBulk::getDb()->beginTransaction();
    ...
    public function actionDispatch_car(){
        ...
        if($order->order_status != 2){
            $data = $this->encrypt(['code'=>400,'msg'=>'订单已分派']);
            return $this->resultInfo($data);
        }
        $order->driverinfo = json_encode([['id'=>'','price'=>$order->line_price,'carnumber'=>$carnumber,'contant'=>$driver_name,'tel'=>$mobile]],JSON_UNESCAPED_UNICODE);
        $res = $order->save();
