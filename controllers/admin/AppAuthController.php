...
class AppAuthController extends AdminBaseController
{
    /**
     * Desc: 权限列表
     * Created by pysh
    ...
    public function actionIndex(){
        ...
        if($this->request->isAjax){
            $list = AppAuthApp::find()->where(['parent_id'=>$pid]);
            $count = $list->count();
            $list = $list
    ...
    public function actionEdit(){
        ...
        if($this->request->isPost){
            ...
            //     $have = AppAuthApp::find()->where(['route'=>$this->request->bodyParams['route']])
            //         ->andWhere(["<>",'id',$id])
            //         ->asArray()
            //         ->all();
            ...
            // }
            // 更新
            $permission = AppAuthApp::findOne(['id'=>$id]);
            ...
            if ($flag_error) {
                if($permission->save()){
                    AddLogController::addSysLog(AddLogController::app_auth,'修改权限,权限为:'.$permission->display_name);
                    return $this->withSuccess('修改成功!!')->redirect(route('admin.app-auth.index','pid='.$this->request->bodyParams['parent_id']));
                } else {
                    $this->withErrors('保存失败!');
                }
            }
        }
        $info = AppAuthApp::find()->where(['id'=>$id])->asArray()->one();
    ...
    public function actionAdd(){
        ...
        if($this->request->isPost){
            ...
            if ($flag_error) {
                ...
                AddLogController::addSysLog(AddLogController::app_auth,'新增权限,权限为:'.$permission->display_name);
                return $this->withSuccess('新增成功!')->redirect(route('admin.app-auth.index','pid='.$this->request->bodyParams['parent_id']));
            }
        }
    ...
    public function actionDel(){
       if($this->request->isAjax){
            ...
            $model = AppAuthApp::findOne(['id'=>$id]);
            $children = AppAuthApp::find()
                ->where(['parent_id'=>$id])
                ->asArray()
                ->all();
            if ($children) {
            ...
            if($model && $model->delete()){
                AddLogController::addSysLog(AddLogController::app_auth,'刪除权限,权限 id 为:'.$id);
