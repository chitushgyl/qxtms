...
<div class="layui-card">
    <div class="layui-card-header layuiadmin-card-header-auto">
        <div class="layui-btn-group ">
            <?php if(can('admin.app-level-carriage.add')){?>
            <a class="layui-btn layui-btn-sm" href="<?php echo route('admin.app-level-carriage.add','pid='.$pid);?>">添 加</a>
            <?php }?>
        </div>
    </div>
    <div class="layui-card-body">
        ...
        <script type="text/html" id="options">
            <div class="layui-btn-group">
                <?php if(can('admin.app-level-carriage.edit')){?>
                <a class="layui-btn layui-btn-sm" lay-event="edit">编辑</a>
                <?php }?>

                <?php if(can('admin.app-level-carriage.permission')){?>
...
<script>
    layui.use(['layer','table','form'],function () {
        ...
        var dataTable = table.render({
            elem: '#dataTable'
            ,height: 500
            ,url: "<?php echo route('admin.app-level-carriage.index')?>"//数据接口
            ,page: true //开启分页
            ,cols: [[ //表头
            ...
            if(layEvent === 'del'){
                layer.confirm('确定刪除该权限吗？', function(index){
                    ...
                    $.ajax({
                        ...
                        url:"<?php echo  route('admin.app-level-carriage.del') ?>",
                        dataType:"JSON",
                        type:"POST",
                        beforeSend:function(){
                ...
                location.href = '/admin/app-level-carriage/edit?id='+data.level_id;
            }  else if (layEvent === 'permission'){
                location.href = '/admin/app-level-carriage/permission?id='+data.level_id;
            }
        });
    })
</script>
