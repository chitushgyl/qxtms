...
<script type="text/html" id="options">
    <div class="layui-btn-group">
        <?php if(can('admin.carriage.del')){?>
            <a class="layui-btn layui-btn-danger layui-btn-sm " lay-event="del">删除</a>
        <?php } ?>
        <?php if(can('admin.carriage.view')){?>
...
<script>
    layui.use(['layer','table','form'],function () {
        ...
        var dataTable = table.render({
            elem: '#dataTable'
            ,height: 500
            ,url: "<?php echo route('admin.carriage.index'); ?>" //数据接口
            // ,response:{
            //     dataName:'list',
            //     countName:'counts'
            // }
            ,page: true //开启分页
            ,cols: [[ //表头
                {field: 'cid', title: 'ID',width:80, align:'center',hide:true}
                ,{field: 'name', title: '承运公司名称', align:'center',width:150}
        ...
        table.on('tool(dataTable)', function(obj){ //注：tool是工具条事件名，dataTable是table原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'del'){
                layer.confirm('确认刪除吗？', function(index){
                    $.ajax({
                        data:{_csrf:"<?php echo Yii::$app->request->csrfToken; ?>",id:data.cid},
                        url:"<?php echo route('admin.carriage.del'); ?>",
                        dataType:"JSON",
                        type:"POST",
                        beforeSend:function(){
                ...
                location.href = '/admin/carriage/view?id='+data.cid;
            }
        });
