...
if($pid==0){
    ...
    $parent_id = AppAuthApp::findOne(['id'=>$pid])->parent_id;
}
?>
<div class="layui-card">
    <div class="layui-card-header layuiadmin-card-header-auto">
        <div class="layui-btn-group ">
            <?php if(can('admin.app-auth.add')){?>
            <a class="layui-btn layui-btn-sm" href="<?php echo route('admin.app-auth.add','pid='.$pid);?>">添 加</a>
            <?php }?>
    ...
    <div class="layui-card-body">
        ...
        <script type="text/html" id="options">
            <div class="layui-btn-group">
                ...
                <?php if(can('admin.app-auth.edit')){?>
                <a class="layui-btn layui-btn-sm" lay-event="edit">编辑</a>
                <?php }?>
                <?php if(can('admin.app-auth.del')){?>
...
<script>
    ...
    layui.use(['layer','table','form'],function () {
        ...
        var dataTable = table.render({
            elem: '#dataTable'
            ,height: 500
            ,url: "<?php echo route('admin.app-auth.index')?>"+"pid="+pid //数据接口
            ,page: true //开启分页
            ,cols: [[ //表头
            ...
            if(layEvent === 'del'){
                layer.confirm('确定刪除该权限吗？', function(index){
                    ...
                    $.ajax({
                        ...
                        url:"<?php echo  route('admin.app-auth.del') ?>",
                        dataType:"JSON",
                        type:"POST",
                        beforeSend:function(){
                ...
                location.href = '/admin/app-auth/edit?id='+data.id;
            } else if (layEvent === 'children'){
                console.log(data.id);
                console.log(data.parent_id);
                $('#returnParent').attr('pid',data.id);
                location.href = '/admin/app-auth/index?pid='+data.id;
            }
        });
        ...
        $("#returnParent").click(function () {
            ...
            location.href = '/admin/app-auth/index?pid='+ pid;
        })
    })
</script>
